...
#"""**********************************Main Menu********************************************"""
        ...
        self.frame_menu.config(relief=RIDGE, padding=(70, 15))


        ttk.Button(
            self.frame_menu, text="Tải ảnh lên", command=self.upload_action).grid(row=0, column=0, padx=5, pady=5, sticky='sw')

        ttk.Button(
            self.frame_menu, text="Cắt ảnh", command=self.crop_action).grid(row=1, column=0, padx=5, pady=5, sticky='sw')

        ttk.Button(
            self.frame_menu, text="Thêm văn bản", command=self.text_action_1).grid(row=2, column=0, padx=5, pady=5, sticky='sw')

        ttk.Button(
            self.frame_menu, text="Vẽ ảnh", command=self.draw_action).grid(row=3, column=0, padx=5, pady=5, sticky='sw')

        ttk.Button(
            self.frame_menu, text="Làm mờ khuôn mặt", command=self.blurFace_action).grid(row=4, column=0, padx=5, pady=5, sticky='sw')

        ttk.Button(
            self.frame_menu, text="Làm mờ/trơn ảnh", command=self.blur_action).grid(row=5, column=0, padx=5, pady=5, sticky='sw')

        ttk.Button(
            self.frame_menu, text="Chỉnh sáng", command=self.adjust_action).grid(row=6, column=0, padx=5, pady=5, sticky='sw')

        ttk.Button(
            self.frame_menu, text="Xoay ảnh", command=self.rotate_action).grid(row=7, column=0, padx=5, pady=5, sticky='sw')

        ttk.Button(
            self.frame_menu, text="Lật ảnh", command=self.flip_action).grid(
            row=8, column=0, padx=5, pady=5, sticky='sw')
        ttk.Button(
            self.frame_menu, text="Ảnh vẽ", command=self.sketch_action).grid(row=9, column=0, padx=5, pady=5, sticky='sw')
        
        ttk.Button(
            self.frame_menu, text="Lưu ảnh", command=self.save_action).grid(row=10, column=0, padx=5, pady=5, sticky='sw')
...
#"""**********************************Triggered Function for Main Menu Options********************************************"""
    ...
    def blurFace_action(self):
    	...
    	# step-5: addition
    	self.filtered_image = cv2.add(fg_img,bg_img_blur)
    	self.display_image(self.filtered_image)
...
#"""**********************************Adjust Menu Options********************************************"""
    def adjust_action(self):
        ...
        self.brightness_slider = Scale(
            self.side_frame, from_=0, to_=255,  resolution=1, orient=HORIZONTAL, command=self.brightness_action)
...
#"""**********************************Triggers for Adjust Menu Options********************************************"""
    def brightness_action(self, value):
        ...
        bar = value - 127
        if bar >= 0:
        	v = np.ones_like(self.edited_image,dtype='uint8')*bar
        	self.filtered_image = cv2.add(self.edited_image,v)
        
        else:
        	value = 127 - value
        	v = np.ones_like(self.edited_image,dtype='uint8')*value
        	self.filtered_image = cv2.subtract(self.edited_image,v)
        self.display_image(self.filtered_image)
...
#"""**********************************Pencil sketch**********************************************************************************"""
    def sketch_action(self):
        self.refresh_side_frame()
        ttk.Label(
			self.side_frame,text="ksize").grid(row=0,column=2,padx=5, pady=5, sticky='sw')
        self.ksize_slider = Scale(
			self.side_frame, from_=1, to_=100,  resolution=1, orient=HORIZONTAL, command=self.ksize_action)
        self.ksize_slider.grid(row=1, column=2, padx=5,  sticky='sw')
        self.ksize_slider.set(1)

        ttk.Label(
			self.side_frame,text="gamma").grid(row=2,column=2,padx=5, pady=5, sticky='sw')
        self.gamma_slider = Scale(
			self.side_frame, from_=1, to_=100,  resolution=1, orient=HORIZONTAL, command=self.gamma_action)
        self.gamma_slider.grid(row=3, column=2, padx=5,  sticky='sw')
        self.gamma_slider.set(1)


    def ksize_action(self, k):
        k = self.ksize_slider.get()
        k = 2*k+1
        ...
        self.filtered_image = cv2.LUT(division_img.astype('uint8'),lut.astype('uint8'))

        self.display_image(self.filtered_image)

    def gamma_action(self, g):
        k = self.ksize_slider.get()
        k = 2*k+1
        g = self.gamma_slider.get()
        g = g/100
